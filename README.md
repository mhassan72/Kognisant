...
Kognisant is a terminal-native AI assistant designed for software engineers who want model-agnostic, privacy-conscious tooling without external bloat. Built entirely on the Python standard library, it requires zero runtime dependencies and runs on Python 3.10+ (POSIX platforms: Linux, macOS).
...
- **Resilient & Safe** — Checkpoint rollbacks, directory-traversal protection, symlink containment, and exponential-backoff API retries keep your sessions stable.
- **Production-Hardened Daemon** — A forked background execution engine with atomic writes, crash recovery, cron scheduling, and PID-reuse-safe orphan cleanup.
...
│   ├── tools.py                   # Tool specifications & execution (file, web, browser, search, jobs)
│   ├── daemon.py                  # Background daemon: fork, FD cleanup, main loop, stream readers, orphan cleanup
│   ├── jobs.py                    # Versioned job queue: atomic writes, FileLock, CronParser, MigrationRegistry
│   ├── scripts.py                 # Atomic script CRUD with symlink containment
│   ├── sdd.py                     # Spec-Driven Development orchestration
...
> **Note:** The background daemon and job scheduling features require a POSIX-compliant OS (Linux or macOS). The interactive chat and other features work on any platform with Python 3.10+.
...
Kognisant includes a production-hardened background daemon that runs scripts autonomously — bots, cron jobs, monitoring tasks, and AI agent work — without keeping a terminal open. The daemon requires a POSIX-compliant system (Linux or macOS).

### Daemon Management

```bash
kognisant daemon start      # Fork daemon to background
kognisant daemon stop       # Graceful shutdown (SIGTERM → wait 10s → SIGKILL)
kognisant daemon restart    # Stop + start in one command
kognisant daemon status     # PID, uptime, active jobs, last poll time
```
...
kognisant job list           # Name, type, state, run count, exit code, next run
kognisant job cancel <name>  # Cancel a pending/running job
kognisant job remove <name>  # Permanently remove (terminates if running)
kognisant job edit <name> --cron "0 3 * * *"  # Edit in-place
kognisant job logs <name>    # View last 50 lines of output
kognisant job logs <name> --follow  # Tail live output (500ms polling)
```

### Cron Scheduling (UTC)

All cron expressions use the standard 5-field format and are evaluated in UTC:
...
Supports `*`, commas, ranges, and steps (e.g., `*/15 * * * *`). Unmatchable expressions (like Feb 31st) produce a validation warning.

### Scheduler Policies

| Policy | Behavior |
|--------|----------|
| `skip` (default) | Missed executions during clock jumps are silently discarded |
| `catchup_once` | Each missed job fires exactly once after a clock jump |

### Crash Recovery & Atomic Writes

- Every write to `jobs.json` uses an atomic sequence: tempfile → fsync → rename → backup
- If `jobs.json` is corrupted or deleted, the daemon auto-recovers from `jobs.json.bak`
...
| Python `SyntaxError` on launch | Python version is below 3.10. | Upgrade to Python 3.10 or newer. |
...
Please ensure your changes stay within the **Python 3.10+ standard library** to preserve the zero-dependency promise.
